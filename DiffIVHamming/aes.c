...
This script:
- Retains identical key and plaintext values
- Measures hamming distances across bit flips in IVs
...
int main(int argc, char *argv[]) {
    ...
    for (int t = 0; t < NUMTRIALS; t++) {
        ...
        for (int s = 0; s < SUBTRIALS; s++) {
            uint8_t flipped[16];
            memcpy(flipped, iv, 16);
            flipped[s / 8] ^= 1 << (s % 8);

            uint8_t *flipbuf;
            aescbcencrypt(&flipbuf, plaintext, PLAINTEXTSIZE, key, flipped);
