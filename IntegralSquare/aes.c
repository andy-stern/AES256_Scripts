...
int main(int argc, char *argv[]) {
    ...
    fprintf(fcsv, "nr,trial,balanced_bytes,fully_balanced,key_hex,base_plaintext_hex\n");

    for (int t = 0; t < NUMTRIALS; t++) {
        uint8_t key[32], base[16];
        fread(key, 1, sizeof(key), urand);
        fread(base, 1, sizeof(base), urand);
        ...
        for (int v = 0; v < 256; v++) {
            ...
            pt[ACTIVE] = (uint8_t)v;
            uint8_t ct[16];
            encryptblock(ct, pt, roundkeys);
            for (int i = 0; i < 16; i++) xorsum[i] ^= ct[i];
        }
        free(roundkeys);

        int balanced = 0;
        for (int i = 0; i < 16; i++) if (xorsum[i] == 0) balanced++;
        int full = (balanced == 16) ? 1 : 0;

        fprintf(fcsv, "%d,%d,%d,%d,", NR, t + 1, balanced, full);
        writehex(fcsv, key, sizeof(key));
        fputc(',', fcsv);
        writehex(fcsv, base, sizeof(base));
        fputc('\n', fcsv);
    }
