/*
gcc -O3 -march=native -funroll-loops -fopenmp aes.c -o aes

This script:
- Retains identical IV and key values
- Measures avalanche matrices across bit flips in plaintexts
*/
...
static uint16_t counts[4096][4096];
...
int main(int argc, char *argv[]) {
    ...
    char path[128];
    snprintf(path, sizeof(path), "%s/trials.csv", foldername);
    FILE *fcsv = fopen(path, "w");
    if (!fcsv) {
        perror("trials.csv");
        return 1;
    }
    fprintf(fcsv, "nr,trial,key_hex,iv_hex,plaintext_hex,ciphertext_hex\n");
    ...
    for (int t = 0; t < NUMTRIALS; t++) {
        ...
        fputc(',', fcsv);
        writehex(fcsv, ciphertext, PLAINTEXTSIZE);
        fputc('\n', fcsv);
        ...
        for (int s = 0; s < SUBTRIALS; s++) {
            ...
            for (int j = 0; j < SUBTRIALS; j++) {
                if ((ciphertext[j / 8] ^ flippedciphertext[j / 8]) & (1 << (j % 8))) {
                    counts[s][j]++;
                }
            }
        }
    ...
    snprintf(path, sizeof(path), "%s/avalanche_counts.bin", foldername);
    FILE *fmatrix = fopen(path, "wb");
    fwrite(counts, sizeof(uint16_t), 4096 * 4096, fmatrix);
    fclose(fmatrix);
