...
This script:
- Retains identical IV and plaintext values
- Measures hamming distances across bit flips in keys
*/
...
int main(int argc, char *argv[]) {
    ...
    for (int t = 0; t < NUMTRIALS; t++) {
        ...
        for (int s = 0; s < SUBTRIALS; s++) {
            uint8_t flipped[32];
            memcpy(flipped, key, 32);
            flipped[s / 8] ^= 1 << (s % 8);

            uint8_t *flipbuf;
            aescbcencrypt(&flipbuf, plaintext, PLAINTEXTSIZE, flipped, iv);
