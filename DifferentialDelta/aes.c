...
This script:
- Builds a Lambda-set: 256 plaintexts, byte 0 takes all values 0..255, rest constant
- Encrypts the set (single-block ECB) under a random key
- Checks the integral (Square) property: XOR of each output byte over the set
*/
...
int main(int argc, char *argv[]) {
    ...
    fprintf(fcsv, "nr,trial,out_diff_weight,key_hex,plaintext_hex,out_diff_hex\n");

    uint8_t DELTA[16] = {0};
    DELTA[0] = 1;

    for (int t = 0; t < NUMTRIALS; t++) {
        uint8_t key[32], plaintext[16], plaintext2[16];
        fread(key, 1, sizeof(key), urand);
        fread(plaintext, 1, sizeof(plaintext), urand);

        for (int i = 0; i < 16; i++) {
            plaintext2[i] = plaintext[i] ^ DELTA[i];
        }
        ...
        uint8_t c1[16], c2[16];
        encryptblock(c1, plaintext, roundkeys);
        encryptblock(c2, plaintext2, roundkeys);
        free(roundkeys);
        ...
        int weight = hamming(c1, c2, 16);

        fprintf(fcsv, "%d,%d,%d,", NR, t + 1, weight);
        writehex(fcsv, key, sizeof(key));
        fputc(',', fcsv);
        writehex(fcsv, plaintext, sizeof(plaintext));
        fputc(',', fcsv);
        writehex(fcsv, outdiff, sizeof(outdiff));
        fputc('\n', fcsv);
    }
