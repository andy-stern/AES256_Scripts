...
This script:
- Has different IVs, keys, and plaintexts
- Outputs in chunks of 4096 bytes
- Has an abitrary NR (rounds) input
*/
...
/* CONF */
const size_t PLAINTEXTCHUNK = 4096;        
...
int main(int argc, char *argv[]) {
    if (argc != 3) {
        printf("Pass args in the format ./aes NR OUTPUTSIZEBYTES");
        return 1;
    }
    ...
    size_t TARGETSIZE = strtoull(argv[2], NULL, 10);
    if (TARGETSIZE <= 0) {
        printf("Invalid target size. Outputs in chunks of 4096.");
        return 1;
    }
    if (TARGETSIZE % PLAINTEXTCHUNK != 0) {
        TARGETSIZE = ((TARGETSIZE / PLAINTEXTCHUNK) + 1) * PLAINTEXTCHUNK;
    }
    ...
    char path[128];
    FILE *fplain, *fkey, *fiv, *fcipher;

    snprintf(path, sizeof(path), "%s/plaintext.bin", foldername);
    fplain = fopen(path, "wb");

    snprintf(path, sizeof(path), "%s/key.bin", foldername);
    fkey = fopen(path, "wb");

    snprintf(path, sizeof(path), "%s/iv.bin", foldername);
    fiv = fopen(path, "wb");

    snprintf(path, sizeof(path), "%s/ciphertext.bin", foldername);
    fcipher = fopen(path, "wb");
    ...
    while (written < TARGETSIZE) {
        ...
        fread(plaintext, 1, sizeof(plaintext), urand);
        ...
        uint8_t *enc;
        int enclen = aescbcencrypt(&enc, plaintext, sizeof(plaintext), key, iv);

        fwrite(plaintext, 1, sizeof(plaintext), fplain);
        fwrite(key, 1, sizeof(key), fkey);
        fwrite(iv, 1, sizeof(iv), fiv);
        fwrite(enc, 1, enclen, fcipher);
